...
function plot_data_triatomic_system(Z1::Int, Z2::Int, 
    θ1::Real, θ2::Real, r1::Real, r2::Real, exact_θ::Real, exact_r::Real)
    ...
    println(@sprintf "Bond length: (model) %12.3lf (exact) %12.3lf " all_separations[curr_min[1]] exact_r )
    println(@sprintf "Bond angle : (model) %12.3lf (exact) %12.3lf " all_angles[curr_min[2]] exact_θ)
...
function plot_h2o_comparison()
    ...
    all_angles, all_separations, energy = 
        plot_data_triatomic_system(Z1,Z2,θ1,θ2,r1,r2,H2O_angle,H2O_bond_lenght)
    ...
    p = contourf(all_angles, all_separations, energy,
        xlabel=x_label, ylabel=y_label,levels=18,
        clims=(0,12),colorbar_title=c_label, color=:viridis)
    
    contour!(all_angles, all_separations, energy,
        xlabel=x_label,levels=18,clims=(0,12),
...
function plot_co2_comparison()
    ...
    all_angles, all_separations, energy = 
        plot_data_triatomic_system(Z1,Z2,θ1,θ2,r1,r2,CO2_angle,CO2_bond_lenght)
    ...
    p = contourf(all_angles, all_separations, energy,
        xlabel=x_label, ylabel=y_label,levels=18,
        clims=(0,24),colorbar_title=c_label,color=:viridis)
...
function plot_o3_comparison()
    ...
    all_angles, all_separations, energy = 
        plot_data_triatomic_system(Z1,Z1,θ1,θ2,r1,r2,CO2_angle,CO2_bond_lenght)

    x_label = get_element_symbol(Z1) * "-" * get_element_symbol(Z1) *
        "-" * get_element_symbol(Z1) * " angle [degrees]"
    y_label = get_element_symbol(Z1) * "-" * get_element_symbol(Z1) *
        " length\n[Ångstrom]"
    c_label = "ΔE [eV]"
...
function plot_triatomic_comparisons()
    p1 = plot_o3_comparison()
    p2 = plot_h2o_comparison()
    p3 = plot_co2_comparison()
